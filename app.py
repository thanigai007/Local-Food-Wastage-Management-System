...
menu = st.sidebar.radio("Menu", ["Project Introduction", "View Tables", "CRUD Operations", "Make a Claim", "SQL Queries & Visualization", "Learner SQL Queries", "About me"])
...
# CRUD Operations
    ...
    table_choice = st.selectbox("Choose Table", ["food_listings", "providers", "receivers"])
    ...
    if table_choice == "food_listings":
        if crud_option == "Create":
            ...
            if st.button("Add"):
                try:
                    cursor.execute("""
                        INSERT INTO food_listings (Provider_ID, Food_Type, Quantity, Location, Contact, Expiry_Date, Description, Image_URL)
                        VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                    """, (provider_id, food_type, quantity, location, contact, expiry, description, image_url))
                    conn.commit()
            ...
            column = st.selectbox("Column", ["Food_Type", "Quantity", "Location", "Contact", "Expiry_Date", "Description", "Image_URL"])
    ...
    receiver_id = st.number_input("Enter Your Receiver ID", min_value=1)
